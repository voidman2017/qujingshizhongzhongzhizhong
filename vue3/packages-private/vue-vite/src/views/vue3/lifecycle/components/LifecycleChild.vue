<template>
    <div>
        ...
        <button @click="addCount">click {{ count }} times</button>
    </div>
</template>
...
lifecycles.forEach((lifecycle) => {
    lifecycle.fn(() => {
        lifecycle.log && console.log(`child - ${lifecycle.name} is running`)
        if (!lifecycle.effect) {
            lifecyclesRun.value.push(`child - ${lifecycle.name}`) // 其中 onUpdated、 onRenderTracked、 onRenderTriggered 如果产生视图更新，则会导致循环执行
            emit('lifeCycleRun', `${lifecycle.name}`)
        }
    })
})
